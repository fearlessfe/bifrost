...
| `pricing_patch` | string | No | JSON-encoded pricing fields to override, or a standalone `cost_multiplier` in `(0, 1]` |
...
`cost_multiplier` applies a negotiated contract multiplier to the final catalog-computed request cost. For example, `0.8` bills at 80% of the base cost. It cannot be combined with explicit pricing fields in the same override.

Provider-reported authoritative costs are used as returned and are not multiplied again. This prevents costs already computed upstream or attached by an earlier streaming stage from being discounted twice.
...
Apply a 20% contract discount to the selected OpenAI request types:

```json
{
  "id": "openai-contract-discount",
  "name": "OpenAI contract discount",
  "scope_kind": "provider",
  "provider_id": "openai",
  "match_type": "wildcard",
  "pattern": "*",
  "request_types": ["chat_completion", "text_completion", "responses", "embedding", "image_generation"],
  "pricing_patch": "{\"cost_multiplier\":0.8}"
}
```
